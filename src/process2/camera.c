...
void	ini_ray(t_pro *p)
{
	...
	while (newan <= g_angle + 30)
	{
		p->mlx.tline[ladex].costart.x = g_xstart;
		p->mlx.tline[ladex].costart.y = g_ystart;
		p->mlx.tline[ladex].costop.x = 0;
		p->mlx.tline[ladex].costop.y = 0;
		// draw_line(&p->mlx, p->mlx.tline, newan, ladex);
		newan += 10;
		ladex++;
	}
		printf("here\n");
