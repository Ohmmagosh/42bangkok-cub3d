...
/*   Updated: 2023/05/04 04:40:15 by rchiewli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	hook(int key, t_pro *p)
{
	...
	{
		if(p->map.map[(int)(p->tline.pos.y)][(int)(p->tline.pos.x + p->di.x * (step * 2))] != '1' \
		&& p->map.map[(int)(p->tline.pos.y)][(int)(p->tline.pos.x + p->di.x * (step * 2))] != 'D')
			p->tline.pos.x += p->di.x * step;
		if(p->map.map[(int)(p->tline.pos.y + p->di.y * (step * 2))][(int)(p->tline.pos.x)] != '1' \
		&& p->map.map[(int)(p->tline.pos.y + p->di.y * (step * 2))][(int)(p->tline.pos.x)] != 'D')
			p->tline.pos.y += p->di.y * step;
	}
	else if (key == 0)
	{
		if (p->map.map[(int)(p->tline.pos.y - p->di.x * (step * 2))][(int)(p->tline.pos.x + p->di.y * (step * 2))] != '1')
		{
